...
(function () {
  ...
  function initRemoteSettings() {
    ...
    window.RemoteConfig.initRemoteConfig({
      ...
      // Стартовый запас подсказок правится ЗДЕСЬ, а не при создании сейва:
      // раньше этой секунды значение ключа неизвестно. Зовём и после провала
      // загрузки — без конфига поправка нулевая, но флаг проставится, иначе
      // сейв ждал бы ответа, которого не будет. Подробности — в storage.js.
      window.Storage.applyStartGrant();
