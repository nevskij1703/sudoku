...
**Два ключа из девятнадцати устроены не как остальные.**

`hearts_per_level` записывается в НОВЫЙ УРОВЕНЬ полем `heartsAtStart`, и по нему
— а не по текущей настройке — считаются «уровень нетронут» (`isUntouched`) и
шкала сердец. Настройка крутится из облака и может измениться между запусками:
сравнение с живым значением показало бы потерянное сердце там, где его не
теряли, и наоборот — спрятало бы настоящую потерю. **Не возвращай сюда
`CFG.BALANCE.heartsPerLevel`.**

`hints_start` правит поле сейва и правит его ОДИН раз, **разницей** к значению
сборки: `applyStartGrant` в `storage.js`, флаг `startAdjusted`,
`migrations[10]`. Сейв рождается первым шагом `init()`, раньше ответа сети, и
ключ, прочитанный при создании, вернул бы значение сборки каждому новому игроку
— то есть ровно тем, ради кого он заведён.
